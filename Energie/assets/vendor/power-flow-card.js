...
class PowerFlowCard extends LitElement {
  ...
  constructor() {
    ...
    this.svgPaths = {
      ...
      wallbox: "data:image/svg+xml;base64,",
    ...
    this.lineConfig = [
      ...
      // Symcon-Anpassung:
      // Der primary-CONTAINER wird für die Wallbox genutzt.
      // Die Geometrie stammt aber aus dem eigenen "wallbox"-SVG-Pfad
      // und nicht mehr aus der früheren Netz-Eingangsleitung.
      {
        id: "ev",
        type: "ev",
  ...
  async loadSVG(path, containerEl, lineType, isBackground) {
    try {
      ...
      if (isBackground) {
        // Symcon-Anpassung:
        // Das originale Hintergrund-SVG enthält bereits statische
        // Energie-Leitungen. Diese würden zusätzlich zu unseren
        // dynamischen Leitungen sichtbar bleiben.
        //
        // Daher entfernen wir ALLE eingebauten powerline-* Gruppen
        // aus dem Hintergrund und behalten nur Haus, Dach, Fenster,
        // Solarflächen, Batterie, Auto usw.
        const parser = new DOMParser();
        const doc = parser.parseFromString(text, "image/svg+xml");

        doc.querySelectorAll('[id^="powerline-"]').forEach((el) => el.remove());

        const cleanedSvg = doc.documentElement;
        containerEl.innerHTML = cleanedSvg.outerHTML;
      } else {
  ...
  static get styles() {
    return css`
      ...
      /* Symcon: Originale, statische Energiepfade des Hintergrund-SVG
         dürfen niemals sichtbar sein. */
      #svg-container-bg [id^="powerline-"] {
