...
mit Grafik.txt
Dokument
Kriegen wir dei Grafik des Hauses soweit, dass sie sich auf dm ganzen html-Feld verteilt, ohne die Skalierung zu verändern. Die höhe ist ja nicht das Problem, nur die Breite. Kriegst du das hin das das ordungsgemäss skliert auch mit den Infoboxen dass diese am rechten Ort bleiben


Eingefügter Text(86).txt
Dokument
funkioniert nicht, schaue mal wie es das snkey macht


V1(1).txt
Dokument
Mach den Umaschaltbutton untenr, dann geht es wie beim snkey. Das Problem ist oben in er Grafik wird die maus nicht merh erkannt. Bau das hier ein, das war die letzte gute version ohne umschaltung
...
src = Path("/mnt/data/module_farben_batterieleitung_fix.php")
dst = Path("/mnt/data/module_farben_batterierichtung_fix.php")
text = src.read_text(encoding="utf-8")

# Insert explicit battery flow color/class fix before optional layers.
marker = """    function applyPfcOptionalLayers(d, batteries, wallbox) {
"""
helper = """    function applyPfcBatteryFlowColor(batteryTotal) {
...
if marker not in text:
    raise RuntimeError("applyPfcOptionalLayers marker nicht gefunden.")
text = text.replace(marker, helper + marker, 1)

# Call fix immediately after hass update and again after updateFlow.
needle = """        pfcCard.hass = { states };

        updatePfcInfoCards(d, grid, haus, pvs, batteries, wallbox);"""
replacement = """        pfcCard.hass = { states };

        applyPfcBatteryFlowColor(batteryTotal);
        updatePfcInfoCards(d, grid, haus, pvs, batteries, wallbox);"""
if needle not in text:
    raise RuntimeError("pfcCard.hass block nicht gefunden.")
text = text.replace(needle, replacement, 1)

needle2 = """                pfcCard.updateFlow();
                applyPfcOptionalLayers(d, batteries, wallbox);"""
replacement2 = """                pfcCard.updateFlow();
                applyPfcBatteryFlowColor(batteryTotal);
                applyPfcOptionalLayers(d, batteries, wallbox);"""
if needle2 not in text:
    raise RuntimeError("requestAnimationFrame updateFlow block nicht gefunden.")
text = text.replace(needle2, replacement2, 1)

dst.write_text(text, encoding="utf-8")

res = subprocess.run(["php", "-l", str(dst)], capture_output=True, text=True)
print(res.stdout.strip() or res.stderr.strip())
print(f"Erstellt: {dst} ({dst.stat().st_size} Bytes)")
STDOUT/STDERR
No syntax errors detected in /mnt/data/module_farben_batterierichtung_fix.php
Erstellt: /mnt/data/module_farben_batterierichtung_fix.php (82432 Bytes)
Jetzt ist die Ursache sauber behoben.

Die power-flow-card unterscheidet die Batterieklassen intern nicht zuverlässig. In einer älteren funktionierenden Variante hatten wir deshalb die Batterie-Leitung nach jedem Update explizit auf bat-charge oder bat-discharge gesetzt. 


Ich habe diesen Mechanismus wieder eingebaut, jetzt aber mit den konfigurierbaren Farben:

Batterie lädt → AC.charge

Batterie entlädt → AC.discharge

Klasse und stroke werden direkt im Shadow-DOM gesetzt

nach updateFlow() wird die Farbe nochmals angewendet, damit die Card sie nicht wieder überschreibt

PHP-Syntaxprüfung: fehlerfrei.

module_farben_batterierichtung_fix.php 


So das sit nicht schlaecht, jetz möchte ich gerne auf der hasugrafik die Box von der Batterie noch etwas höher die sitzt aktuell noch auf der Stromleitung für den Hauseingange

Bild wird erstellt

So das sit nicht schlaecht, jetz möchte ich gerne auf der hasugrafik die Box von der Batterie noch etwas höher die sitzt aktuell noch auf der Stromleitung für den Hauseingange. Kein Bild erstellen sondern ändern
...
src = Path("/mnt/data/module_farben_batterierichtung_fix.php")
dst = Path("/mnt/data/module_batteriebox_hoeher.php")

text = src.read_text(encoding="utf-8")

old = """    #pfc-info-battery {
        left: 55%;
        bottom: 17%;
...
if old not in text:
    # Newer version may already use a variable for the border color.
    old = """    #pfc-info-battery {
        left: 55%;
        bottom: 17%;
        transform: translateX(-50%);
...
if old not in text:
    raise RuntimeError("Batterie-Infobox CSS-Block wurde nicht gefunden.")
...
STDOUT/STDERR
No syntax errors detected in /mnt/data/module_batteriebox_hoeher.php
Erstellt: /mnt/data/module_batteriebox_hoeher.php
Erledigt. Ich habe die Batterie-Infobox in der Hausansicht höher gesetzt:

bottom: 17%;
wurde zu:

bottom: 24%;
Damit sollte sie deutlich oberhalb der Hauseingangsleitung sitzen. Sonst habe ich nichts verändert.

module_batteriebox_hoeher.php 
