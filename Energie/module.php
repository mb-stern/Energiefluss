...
15s nachgedacht
Ja, jetzt so angepasst:

Speicher: weiter nach unten auf exakt dieselbe Höhe wie die Netz-Kachel → bottom: 4%

PV: wieder herunter auf top: 1%

PV dafür deutlich weiter nach links: left: 45%

Die großen Schriften bleiben unverändert.

module_hausansicht_mobile_v5.php


könne die beiden kacheln netz und speicher noc hrunter?

Ja. Ich würde beide noch ein Stück weiter nach unten setzen:

Netz: bottom: 0%

Speicher: ebenfalls bottom: 0%

Damit bleiben beide exakt auf gleicher Höhe.

Alles andere bleibt unverändert.

Schick mir bitte die aktuelle module.php bzw. v5-Datei nochmal hier rein, dann ändere ich exakt diese Version und gebe sie dir direkt zurück.


du hast si e ja oeben gpopstet


Analysiert
from pathlib import Path

src = Path("/mnt/data/module_hausansicht_mobile_v5.php")
dst = Path("/mnt/data/module_hausansicht_mobile_v6.php")

text = src.read_text(encoding="utf-8")

# In v5 Speicher und Netz stehen beide auf bottom: 4%.
# Nur diese beiden Regeln auf 0% setzen.
text = text.replace(
"""    #pfc-info-battery {
        left: 55%;
        bottom: 4%;""",
...
STDOUT/STDERR
Erstellt: /mnt/data/module_hausansicht_mobile_v6.php
Stimmt, die v5 liegt hier noch vor. Ich habe direkt diese Version weiterbearbeitet.

Netz und Speicher stehen jetzt beide noch tiefer auf bottom: 0%. Alles andere bleibt wie in v5.

module_hausansicht_mobile_v6.php 
