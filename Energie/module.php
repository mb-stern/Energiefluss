...
Eingefügter Text(88).txt
Dokument
Die Kachel mit dem Hausverbrauch müsste noch ein wenig weiter nach obendie ist jetzt im Fentster des hauses. Ausserdem wäre eine Handyansicht noch ganz toll, das würde bedeuten dass ganz an den äusseren Radn geganen werdn muss und die Kachel dann grösser weren im Verhältnis
...
src = Path("/mnt/data/module_hausansicht_mobile_v6.php")
dst = Path("/mnt/data/module_hausansicht_mobile_v7.php")

text = src.read_text(encoding="utf-8")

# 1) Responsive Darstellung auch für die nächstgrößere Breite verwenden.
text = text.replace("@media (max-width: 600px) {", "@media (max-width: 900px) {")

# 2) Umschaltbutton links unten statt zentriert.
text = text.replace(
"""        #display-mode-bar {
            padding-top: 3px;
            min-height: 28px;
...
# Auch Desktop/Grundlayout links ausrichten, damit der Button grundsätzlich links unten sitzt.
text = text.replace(
"""    #display-mode-bar {
        flex: 0 0 auto;
        display: flex;
...
# 3) Hausverbrauch zunächst nach oben setzen; exakte Unterkanten-Ausrichtung übernimmt JS.
text = text.replace(
"""    #pfc-info-home {
        right: 7%;
        top: 22%;
...
# 4) Hilfsfunktion zur exakten Unterkanten-Ausrichtung PV <-> Hausverbrauch.
needle = """    function updatePfcInfoCards(d, grid, haus, pvs, batteries, wallbox) {"""
helper = r"""    function alignHomeInfoToSolarBottom() {
...
# Nach dem Aktualisieren der Karten ausrichten.
marker = """        if (gridSub) {
            const energy = [];
...
# safer direct replace only if exact marker exists
if marker in text:
    text = text.replace(marker, replacement)

# Bei Resize erneut exakt ausrichten.
resize_hook = """    window.addEventListener('resize', () => {
        fit();
        requestAnimationFrame(alignHomeInfoToSolarBottom);
    });"""
if "requestAnimationFrame(alignHomeInfoToSolarBottom)" not in text.split("window.addEventListener('resize'")[-1] if "window.addEventListener('resize'" in text else True:
    # inspect common existing resize listener and patch if possible
    text = text.replace(
...
print(f"Erstellt: {dst}")
print("Responsive breakpoint:", "900px" if "@media (max-width: 900px)" in text else "NICHT GEFUNDEN")
print("Button links:", "justify-content: flex-start" in text)
print("Ausrichtungsfunktion:", "function alignHomeInfoToSolarBottom()" in text)
STDOUT/STDERR
Erstellt: /mnt/data/module_hausansicht_mobile_v7.php
Responsive breakpoint: 900px
Button links: True
Ausrichtungsfunktion: True
Ja, so ist es jetzt umgesetzt:

Hausverbrauch nach oben verschoben.

Die Unterkante der Hausverbrauch-Kachel wird automatisch exakt an die Unterkante der PV-Kachel angeglichen – auch wenn sich durch größere Schrift oder zusätzliche Werte die Kachelhöhe ändert.

Umschaltbutton links unten statt mittig.

Die bisherige Handy-Darstellung mit den großen Schriften greift jetzt nicht nur bis 600 px, sondern bis 900 px. Damit bekommt auch die nächstgrößere Ansicht dieselbe Darstellung.

Netz und Speicher bleiben unten wie zuletzt.

PHP-Syntax geprüft: fehlerfrei.

module_hausansicht_mobile_v7.php 



Eingefügter Text(89).txt
Dokument
Jetzt ist was scheifgelaufen totol verzerrt vorher war es so 
...
src = Path("/mnt/data/Eingefügter Text(89).txt")
dst = Path("/mnt/data/module_hausansicht_korrigiert.php")

text = src.read_text(encoding="utf-8")

# 1) Umschaltbutton links unten: nur die horizontale Ausrichtung ändern.
text = text.replace(
"""    #display-mode-bar {
        flex: 0 0 auto;
        display: flex;
...
# 2) Hausverbrauch zunächst nach oben. Die exakte Unterkante wird danach
#    per kleiner Positionsfunktion an die PV-Unterkante angeglichen.
text = text.replace(
"""    #pfc-info-home {
        right: 7%;
        top: 22%;
...
# 3) Exakte Unterkanten-Ausrichtung ergänzen, ohne die Skalierungslogik zu verändern.
needle = "    function updatePfcInfoCards(d, grid, haus, pvs, batteries, wallbox) {"
helper = """    function alignHomeInfoToSolarBottom() {
        const solarInfo = document.getElementById('pfc-info-solar');
        const homeInfo = document.getElementById('pfc-info-home');
...
"""
if "function alignHomeInfoToSolarBottom()" not in text:
    text = text.replace(needle, helper + needle)

# Nach jedem Aktualisieren der Info-Karten ausrichten.
target = """            gridSub.innerHTML = energy.join('<br>');
        }
    }

    function updatePowerFlowCard"""
replacement = """            gridSub.innerHTML = energy.join('<br>');
        }
...
# Bei Größenänderungen nur die Position neu berechnen; fit() selbst bleibt unverändert.
text = text.replace(
"""    window.addEventListener('resize', fit);
...
# WICHTIG: Handyregel bleibt exakt bei 600px. Keine 900px-Regel.
assert "@media (max-width: 600px)" in text
assert "@media (max-width: 900px)" not in text

dst.write_text(text, encoding="utf-8")

print(f"Erstellt: {dst}")
print("600px-Handyregel beibehalten:", "@media (max-width: 600px)" in text)
print("900px-Regel vorhanden:", "@media (max-width: 900px)" in text)
print("Button links:", "justify-content: flex-start" in text)
print("Haus/PV-Unterkante dynamisch ausgerichtet:", "alignHomeInfoToSolarBottom" in text)

# PHP-Syntax prüfen, falls PHP verfügbar ist.
try:
    ...
    print("PHP-Syntaxprüfung nicht verfügbar:", exc)
STDOUT/STDERR
Erstellt: /mnt/data/module_hausansicht_korrigiert.php
600px-Handyregel beibehalten: True
900px-Regel vorhanden: False
Button links: True
Haus/PV-Unterkante dynamisch ausgerichtet: True
No syntax errors detected in /mnt/data/module_hausansicht_korrigiert.php
Ja — der Fehler war die auf 900 px ausgeweitete Handy-Regel. Dadurch wurde bei der nächstgrößeren Ansicht die bereits skalierte Grafik zusätzlich mit riesiger Schrift versehen und wirkte verzerrt. Die von dir hochgeladene „vorher“-Version hatte dagegen die proportionale Skalierung korrekt beibehalten und die große Schrift nur bis 600 px aktiviert. 

 


Ich habe deshalb wieder genau auf dieser guten Version aufgebaut:

keine 900-px-Regel mehr

originale proportionale Skalierung unverändert

Handy-Schrift weiterhin nur bis 600 px

Hausverbrauch nach oben

Unterkante Hausverbrauch automatisch auf Höhe der PV-Unterkante

Umschaltbutton links unten

Netz, Speicher, PV und Wallbox bleiben wie in deiner funktionierenden Version

PHP-Syntax: fehlerfrei

module_hausansicht_korrigiert.php 


doch ich will auch bei grösserere Briet diees Skaliereung
...
src = Path("/mnt/data/module_hausansicht_korrigiert.php")
dst = Path("/mnt/data/module_hausansicht_responsive_v2.php")
...
if old not in text:
    raise RuntimeError("Der erwartete Responsive-Block wurde nicht gefunden.")
...
print(f"Erstellt: {dst}")
print("601–900px-Regel vorhanden:", "@media (min-width: 601px) and (max-width: 900px)" in text)
print("<=600px-Regel vorhanden:", "@media (max-width: 600px)" in text)
...
try:
    ...
    print("PHP-Syntaxprüfung nicht verfügbar:", exc)
STDOUT/STDERR
Erstellt: /mnt/data/module_hausansicht_responsive_v2.php
601–900px-Regel vorhanden: True
<=600px-Regel vorhanden: True
No syntax errors detected in /mnt/data/module_hausansicht_responsive_v2.php
Ja, genau so habe ich es jetzt umgesetzt:

bis 600 px: Schrift weiterhin 200 %

601–900 px: Schrift jetzt 150 %

Die Hausgrafik selbst bleibt weiterhin proportional skaliert und wird nicht verzerrt.

Positionen der Kacheln bleiben wie in der funktionierenden Version.

Hausverbrauch bleibt oben an der PV-Unterkante ausgerichtet.

Umschaltbutton bleibt links unten.

PHP-Syntax geprüft: fehlerfrei.

module_hausansicht_responsive_v2.php 
