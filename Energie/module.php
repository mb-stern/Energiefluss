...
src=Path('/mnt/data/module_energiefluss_sunsynk_v45_wr_leistung_hoeher.php')
if not src.exists():
    src=Path('/mnt/data/module_energiefluss_sunsynk_v44_abstaende_messwerte_compact_lite.php')
text=src.read_text(encoding='utf-8')

old="""                node.style?.setProperty(
                    'transform',
                    'translateY(-14px)',
                    'important'
                );
...
"""
if old not in text:
    raise RuntimeError("Transformblock nicht gefunden")
text=text.replace(old,new,1)
out=Path('/mnt/data/module_energiefluss_sunsynk_v46_wr_leistung_nur_compact_lite.php')
out.write_text(text,encoding='utf-8')
res=subprocess.run(['php','-l',str(out)],capture_output=True,text=True)
print(res.stdout or res.stderr)
print(out)
STDOUT/STDERR
No syntax errors detected in /mnt/data/module_energiefluss_sunsynk_v46_wr_leistung_nur_compact_lite.php

/mnt/data/module_energiefluss_sunsynk_v46_wr_leistung_nur_compact_lite.php
Erledigt.

Die Verschiebung der Wechselrichterleistung wird jetzt nur noch in den kleinen Ansichten angewendet:
...
bleibt die Wechselrichterleistung an ihrer ursprünglichen Position.

Ich habe dazu die Verschiebung an die vorhandene Variable full gekoppelt:
...
Die PHP-Syntax ist fehlerfrei.

Du kannst die angepasste Datei hier herunterladen:

module_energiefluss_sunsynk_v46_wr_leistung_nur_compact_lite.php

Falls der Abstand immer noch nicht genügt, können wir statt 14 px auch 18–20 px nach oben gehen. Das betrifft dann ebenfalls nur die Compact- und Lite-Ansichten. 
...
source = Path("/mnt/data/module_energiefluss_sunsynk_v46_wr_leistung_nur_compact_lite.php")
target = Path("/mnt/data/module_energiefluss_sunsynk_v47_wr_leistung_scope_fix.php")
...
if count != 1:
    raise RuntimeError(
        f"Erwartet 1 Fundstelle für den fehlerhaften full-Block, gefunden {count}"
    )
...
print(target)
STDOUT/STDERR
No syntax errors detected in /mnt/data/module_energiefluss_sunsynk_v47_wr_leistung_scope_fix.php
/mnt/data/module_energiefluss_sunsynk_v47_wr_leistung_scope_fix.php
Der Fehler ist klar: full war nur innerhalb von createSunsynkConfig() definiert und in moveInverterPowerHigher() deshalb nicht verfügbar.

Ich habe die Prüfung auf die globale Layoutvariable umgestellt:
...
Compact / Compact Wide / Lite / Lite Wide: WR-Leistung 14 px höher

Full / Full Wide: Originalposition

Kein Fehler full is not defined mehr

module_energiefluss_sunsynk_v47_wr_leistung_scope_fix.php
...
immer noch nicht du musst nur die Wechselrichterleistung ei nwenig nac hoben dass der abstend iderntish ist wie der von den Spannungen und den Strömen
...
source = Path("/mnt/data/module_energiefluss_sunsynk_v47_wr_leistung_scope_fix.php")
target = Path("/mnt/data/module_energiefluss_sunsynk_v48_wr_leistung_abstand_korrigiert.php")

text = source.read_text(encoding="utf-8")

# Alte Logik entfernen, die zusätzliche Felder erzeugt oder Werte an falscher Stelle verschiebt.
...
pattern = re.compile(
    r"""    function applyInverterPowerDisplay\(card, d\) \{.*?\n    \}\n\n    function moveInverterPowerHigher""",
    re.S
)
text, count = pattern.subn("    function moveInverterPowerHigher", text)
if count != 1:
    raise RuntimeError(
        f"Alte WR-Leistungsfunktion: erwartet 1 Block, gefunden {count}"
    )

# Nur das vorhandene Originalfeld leicht nach oben verschieben.
start = text.index("    function moveInverterPowerHigher(card) {")
end = text.index(
    "\n    function applyAdditionalLoadWattColourByGeometry",
    start
)

new_function = """    function moveInverterPowerHigher(card) {
...
print(target)
STDOUT/STDERR
No syntax errors detected in /mnt/data/module_energiefluss_sunsynk_v48_wr_leistung_abstand_korrigiert.php
/mnt/data/module_energiefluss_sunsynk_v48_wr_leistung_abstand_korrigiert.php
Jetzt ist die alte fehlerhafte Zusatzlogik komplett entfernt.

Die vorhandene Wechselrichterleistung wird in Compact, Compact Wide, Lite und Lite Wide nur um 8 Pixel nach oben verschoben. Dadurch soll der Abstand zu Phase 3 ungefähr gleich gross sein wie die Abstände zwischen Spannung, Phasenströmen und Frequenz.

In Full und Full Wide bleibt die Originalposition unverändert.

module_energiefluss_sunsynk_v48_wr_leistung_abstand_korrigiert.php
